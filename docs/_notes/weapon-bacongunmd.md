...
To implement the Bacon Gun behavior where the bacon projectile wraps an
enemy and immobilizes them (with a visual change), the following updates
will be made:
...
3. Add logic to the BaseEnemy to handle the immobilization and visual
change.
...
bacon_projectile->set("damage", 0.0f); // No direct damage; just
immobilization
...
body->call("apply_bacon_wrap"); // Apply the wrapping effect to the
enemy
...
- Assign the default texture and bacon wrap texture to all enemy
sprites.

- Ensure all enemies inherit from BaseEnemy and have the
apply_bacon_wrap method.

### 3. Weapon Setup:

- Assign the BaconGun to the player and ensure its projectile_scene
points to the BaconProjectile.

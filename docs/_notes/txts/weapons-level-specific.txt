...
the player usualy has a brunch fork in his main hand, but when inside a
level, he can find a specific off hand weapon..

the offhand weapon can be wielded at the same time as the fork if it has
only a shield or debuf effect, but in the off hand
...
(one handed weapons that can be charged for a different atrack)
...
Test each weapon against level enemies to ensure counter logic works as
intended.
